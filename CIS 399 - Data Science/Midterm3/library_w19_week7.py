...
def euclidean_distance(vector1, vector2):  
    ...
    #sqrt(sigma((x-y)^2))
    return math.sqrt(sum([pow((x - y), 2) for x, y in zip(vector1, vector2)]))
...
def knn(row_index, table, columns, k, target):  
    ...
    for j in range(k):
        #append k closest
        check_table = check_table.append(table.loc[distances[j][0], :])

    #get prediction
    return compute_prediction(check_table, target)

def knn_tester(table, columns, k, target):
    prediction_table = []
    tp = 0.0
    tn = 0.0
    fp = 0.0
    ...
    for i in range(len(table)):
        prediction_table.append(tuple([table.loc[i, target], knn(i, table , columns, k, target)]))
        
        #if prediction matches actual
        if prediction_table[i][0] == prediction_table[i][1]:
            if prediction_table[i][0] == 1:
                tp += 1
            else:
                tn += 1
        else:
