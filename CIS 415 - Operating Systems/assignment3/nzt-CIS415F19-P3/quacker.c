...
void * subscriberThread(void *arg) {
    ...
    fprintf(outfile, "<!DOCTYPE html><html><head><title>Subscriber %d Filename: %s</title>Subscriber %d Filename: %s</head><body>", args->threadID, fileName, args->threadID, fileName);
    ...
    while((input = getline(&buffer, &bufferSize, infile)) != -1) {    
        ...
        if(strcmp(argsList[0], "get") == 0) {
            ...
            for(int i = 0; i < TOPICNUM; i++) {
                if(atoi(argsList[1]) == topicStore[i].topicID) {
                    ...
                    if(get == 0) {
                        ...
                        fprintf(outfile, "\n<br><hr><img src=%s width=\'140\' height=\'140\'><br>%s<hr>", nullEntry.photoURL, nullEntry.photoCaption);
                    }
                }
            }
        }
            ...
            printf("Proxy thread %d - type: Subscriber - Executed command: %s %s\n", args->threadID, argsList[0], argsList[1]);
            nanosleep((const struct timespec[]){{(milli / 1000), ((milli % 1000) * 1000000)}}, NULL);
        }
        else if(strcmp(argsList[0], "stop") == 0) {
            printf("Proxy thread %d - type: Subscriber - Executed command: %s\n", args->threadID, argsList[0]);
            break;
        }
