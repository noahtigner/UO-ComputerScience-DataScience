...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            q_val = self.q_vals[(state, action)]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        alpha = self.alpha
        gamma = self.discount
        change = (reward + (gamma * self.getValue(nextState))) - self.getQValue(state, action)

        for feature in features:
            self.weights[feature] = (self.weights[feature] + alpha * features[feature] * change)
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            for w in self.weights.keys():
                print(f"Weight: {w}. Value: {self.weights[w]}")
