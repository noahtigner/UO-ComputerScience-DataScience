...
install.packages(c("twitteR","tm","wordcloud","RColorBrewer","ggmap"),dependencies = TRUE)
...
tweets.uo.df <- read.csv("Tweets_UOgeog_Feb11_19.csv",stringsAsFactors = FALSE)  
...
wordcloud(words=names(word.freq.uo), freq = word.freq.uo, min.freq = 2, random.order = FALSE, colors = brewer.pal(9, "Greens")[-(1:4)])
...
tweets.flu.df <- read.csv("Tweets_Flu_Feb11_19.csv",stringsAsFactors = FALSE)
##Transform the character to Time 
tweets.flu.df$created <- as.POSIXct(tweets.flu.df$created)
##Transform the numeric to character
tweets.flu.df$replyToSID <- as.character(tweets.flu.df$replyToSID)
tweets.flu.df$id <- as.character(tweets.flu.df$id)
tweets.flu.df$replyToUID <- as.character(tweets.flu.df$replyToUID)

##Extract the data between Feb 09 and Feb 10, 2019
tweets.flu.subset <- subset(tweets.flu.df, created >= as.POSIXct('2019-02-09 00:00:00') & created <= as.POSIXct('2019-02-10 23:59:59'))

##Remove the Tweets without latitude and longitude coordinates
tweets.flu.subset <- subset(tweets.flu.subset,longitude != "NA")

##Using leaflet to map the flu data
leaflet(data=tweets.flu.subset) %>%
  addTiles() %>%
